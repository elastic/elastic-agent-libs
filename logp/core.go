...
func createSinkLocal(defaultLoggerCfg Config, outputs ...zapcore.Core) (zapcore.Core, zap.AtomicLevel, *observer.ObservedLogs, map[string]struct{}, error) {
	var (
		sink         zapcore.Core
		observedLogs *observer.ObservedLogs
		err          error
	...
	if defaultLoggerCfg.toObserver {
		...
		sink, err = createLogOutputLocal(defaultLoggerCfg, level)
	}
	if err != nil {
		return nil, level, nil, nil, fmt.Errorf("failed to build log output: %w", err)
	}
...
func ConfigureWithTypedOutputLocal(defaultLoggerCfg, typedLoggerCfg Config, key, value string, outputs ...zapcore.Core) (*Logger, error) {
	sink, level, _, selectors, err := createSinkLocal(defaultLoggerCfg, outputs...)
	if err != nil {
		return nil, err
	}

	var typedCore zapcore.Core
	typedCore, err = createLogOutputLocal(typedLoggerCfg, level)
...
func createLogOutputLocal(cfg Config, enab zapcore.LevelEnabler) (zapcore.Core, error) {
	switch {
	case cfg.toIODiscard:
		return makeDiscardOutput(cfg, enab)
	case cfg.ToStderr:
	...
	case cfg.ToEventLog:
		return makeEventLogOutput(cfg, enab)
	case cfg.ToFiles:
		return makeFileOutputLocal(cfg, enab)
	}
...
func makeFileOutputLocal(cfg Config, enab zapcore.LevelEnabler) (zapcore.Core, error) {
	filename := paths.Resolve(paths.Logs, filepath.Join(cfg.Files.Path, cfg.LogFilenameLocal()))
