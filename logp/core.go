...
func createSink(defaultLoggerCfg Config, outputs ...zapcore.Core) (zapcore.Core, zap.AtomicLevel, map[string]struct{}, error) {
	level := zap.NewAtomicLevelAt(defaultLoggerCfg.Level.ZapLevel())
	return createSinkWithLevel(defaultLoggerCfg, level, outputs...)
}

func createSinkWithLevel(defaultLoggerCfg Config, level zap.AtomicLevel, outputs ...zapcore.Core) (zapcore.Core, zap.AtomicLevel, map[string]struct{}, error) {
	var (
		sink zapcore.Core
		err  error
	)
	...
	if err != nil {
		return nil, level, nil, fmt.Errorf("failed to build log output: %w", err)
	}
	...
	return sink, level, selectors, err
}
...
func ConfigureWithOutputs(defaultLoggerCfg Config, outputs ...zapcore.Core) error {
	sink, level, selectors, err := createSink(defaultLoggerCfg, outputs...)
	if err != nil {
		return err
	}
...
func ConfigureWithTypedOutputLocal(defaultLoggerCfg, typedLoggerCfg Config, key, value string, outputs ...zapcore.Core) (*Logger, error) {
	level := zap.NewAtomicLevelAt(defaultLoggerCfg.Level.ZapLevel())
	return ConfigureWithTypedOutputLocalWithLevel(defaultLoggerCfg, typedLoggerCfg, key, value, level, outputs...)
}

// ConfigureWithTypedOutputLocalWithLevel is like ConfigureWithTypedOutputLocal but accepts a shared AtomicLevel
// so the caller can change the log level at runtime without rebuilding the logger.
func ConfigureWithTypedOutputLocalWithLevel(defaultLoggerCfg, typedLoggerCfg Config, key, value string, level zap.AtomicLevel, outputs ...zapcore.Core) (*Logger, error) {
	root, level, selectors, err := newTypedLoggerCore(defaultLoggerCfg, typedLoggerCfg, key, value, level, outputs...)
	if err != nil {
		return nil, err
	}
...
// ConfigureWithTypedOutputNonGlobal returns a logger that uses typed outputs
// without mutating the global logger.
//
// If a log entry matches the defined key/value, this entry is logged using the
// core generated from `typedLoggerCfg`, otherwise it will be logged by all
// cores in `outputs` and the one generated from `defaultLoggerCfg`.
...
//   - `outputs` is a list of cores that will be added together with the core
//     generated by `defaultLoggerCfg` as the default output for the loggger.
func ConfigureWithTypedOutputNonGlobal(defaultLoggerCfg, typedLoggerCfg Config, key, value string, outputs ...zapcore.Core) (*Logger, error) {
	level := zap.NewAtomicLevelAt(defaultLoggerCfg.Level.ZapLevel())
	return ConfigureWithTypedOutputNonGlobalWithLevel(defaultLoggerCfg, typedLoggerCfg, key, value, level, outputs...)
}

// ConfigureWithTypedOutputNonGlobalWithLevel is like ConfigureWithTypedOutputNonGlobal but accepts a shared AtomicLevel
// so the caller can change the log level at runtime without rebuilding the logger.
func ConfigureWithTypedOutputNonGlobalWithLevel(defaultLoggerCfg, typedLoggerCfg Config, key, value string, level zap.AtomicLevel, outputs ...zapcore.Core) (*Logger, error) {
	root, _, selectors, err := newTypedLoggerCore(defaultLoggerCfg, typedLoggerCfg, key, value, level, outputs...)
	if err != nil {
		return nil, err
	}
	return newLogger(root, selectors), nil
...
// newTypedLoggerCore creates a logger that routes log entries to different outputs based on a key/value field.
// Entries matching the key/value go to the typed output, all others go to the default output.
func newTypedLoggerCore(defaultLoggerCfg, typedLoggerCfg Config, key, value string, level zap.AtomicLevel, outputs ...zapcore.Core) (*zap.Logger, zap.AtomicLevel, map[string]struct{}, error) {
	sink, level, selectors, err := createSinkWithLevel(defaultLoggerCfg, level, outputs...)
	if err != nil {
	...
	typedCore, err := createLogOutput(typedLoggerCfg, level)
	if err != nil {
		return nil, level, nil, fmt.Errorf("could not create typed logger output: %w", err)
	}
	...
	return zap.New(sink, makeOptions(defaultLoggerCfg)...), level, selectors, nil
}
