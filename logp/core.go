...
// ConfigureOTelCore returns beatreceiver suitable logger
// If a log entry contains typedMsg, it is logged using core generated by typedCfg
func ConfigureOTelCore(beatName string, logCfg, typedCfg *config.C, typedMsg string, core zapcore.Core) (*Logger, error) {
	// this is the default config used with beatreceivers
	...
	if logCfg != nil {
		if err := logCfg.Unpack(&loggerConfig); err != nil {
			return nil, fmt.Errorf("error unpacking logging config: %w", err)
		}
	}

	if typedCfg != nil {
		if err := typedCfg.Unpack(&typedConfig); err != nil {
			return nil, fmt.Errorf("error unpacking typed config: %w", err)
		}
	}

	logger, err := ConfigureWithCoreLocal(loggerConfig, core)
	typedCore, err := createLogOutput(typedConfig, logger.Core())
	if err != nil {
		return nil, fmt.Errorf("could not create typed logger output: %w", err)
	}

	sink := &typedLoggerCore{
		defaultCore: logger.Core(),
		typedCore:   typedCore,
		key:         logp.TypeKey,
		value:       logp.EventType,
		message:     typedMsg,
	}

	root := zap.New(sink, makeOptions(loggerConfig)...)
	return newLogger(root, ""), nil
