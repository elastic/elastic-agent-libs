...
func (procStats *Stats) Get() ([]mapstr.M, []mapstr.M, error) {
	...
	for _, process := range plist {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("error converting root map for pid %d: %w", process.Pid.ValueOr(0), err)
		}
...
func (procStats *Stats) pidFill(pid int, filter bool) (ProcState, bool, error) {
	...
	if err != nil {
		return status, true, fmt.Errorf("error while getting GetInfoForPid: %w", err)
	}
	...
	if err != nil {
		return status, true, fmt.Errorf("error while getting FillPidMetrics: %w", err)
	}
